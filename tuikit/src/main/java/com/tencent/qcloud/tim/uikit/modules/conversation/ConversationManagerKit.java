...
public class ConversationManagerKit implements TIMRefreshListener, MessageRevokedManager.MessageRevokeHandler {
    ...
    private final static String SP_ICON_NAME = "icon_name";

    private static ConversationManagerKit instance = new ConversationManagerKit();

    private SharedPreferences mIconNamePreferences;
    ...
    private void init() {
        mIconNamePreferences = TUIKit.getAppContext().getSharedPreferences(SP_ICON_NAME, Context.MODE_PRIVATE);
    ...
    public void loadConversation(IUIKitCallBack callBack) {
        TUIKitLog.i(TAG, "loadConversation ");
        mUnreadTotal = 0;
        ...
        for (int i = 0; i < TIMConversations.size(); i++) {
            ...
            if (conversationInfo != null) {
                ...
                String[] iconName = getIconName(conversationInfo.getId());
                if (iconName != null) {
                    conversationInfo.setTitle(iconName[0]);
                    conversationInfo.setIconUrl(iconName[1]);
                }
    ...
    /**
     * 根据userId，取出本地保存的头像昵称
     *
     * @param userId
     * @return
     */
    private String[] getIconName(String userId) {
        String str = mIconNamePreferences.getString(userId, null);
        if (!TextUtils.isEmpty(str)) {
            int index = str.indexOf("@");
            if (index > 0) {
                String name = str.substring(0, str.indexOf("@"));
                String icon = str.substring(str.indexOf("@") + 1);
                return new String[]{name, icon};
            }
        }
        return null;
    }
