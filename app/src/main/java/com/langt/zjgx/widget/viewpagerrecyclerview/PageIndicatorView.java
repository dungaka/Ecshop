...
public class PageIndicatorView extends LinearLayout {
    ...
    public void initIndicator(int count) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            view.setBackgroundResource(R.drawable.dot_indicator_gray);
            addView(view, params);
            indicatorViews.add(view);
        }
        if (indicatorViews.size() > 0) {
            indicatorViews.get(0).setBackgroundResource(R.drawable.dot_indicator_red);
        }
    }
    ...
    public void setSelectedPage(int selected) {
        ...
        for (int i = 0; i < indicatorViews.size(); i++) {
            if (i == selected) {
                indicatorViews.get(i).setBackgroundResource(R.drawable.dot_indicator_red);
            } else {
                indicatorViews.get(i).setBackgroundResource(R.drawable.dot_indicator_gray);
            }
        }
    }
